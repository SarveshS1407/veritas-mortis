...
import { Play, Square, Mic, AlertCircle, Skull } from "lucide-react";
import { forensicAudio } from "@/lib/forensicAudio";
import { useCrimeAudio } from "@/hooks/useCrimeAudio";

export interface MicroCassetteProps {
  className?: string;
  onPlayStateChange?: (isPlaying: boolean) => void;
}

export default function MicroCassette({
  className = "",
  onPlayStateChange,
}: MicroCassetteProps) {
  ...
  return (
    <div
      className={`relative w-64 h-96 rounded-xl texture-brushed-steel p-4 border border-[#404650] flex flex-col justify-between shadow-[0_20px_50px_rgba(0,0,0,0.9)] select-none transition-all ${className}`}
    >
      ...
      <div className="relative w-full h-36 bg-[#0c0d0f] rounded-md border-2 border-[#20242a] p-2 overflow-hidden shadow-inner flex flex-col justify-between">
        ...
        <div className="relative z-10 w-full h-full bg-[#1e1c18] rounded border border-[#3b362f] p-2 flex flex-col justify-between shadow-md">
          <div className="flex justify-between text-[7px] font-mono text-[#b3a894] px-2 font-bold items-center">
            <span className="text-red-500 font-mono flex items-center gap-1">
              {isPlaying && <Skull size={8} className="animate-pulse text-red-400" />}
              CASE EVIDENCE #09
            </span>
            <span>SIDE A</span>
          </div>

          {/* Spool Drive Mechanism — Spins when playing */}
          <div className="flex justify-around items-center my-1">
            {/* Left Spool */}
            <motion.div
              animate={{ rotate: isPlaying ? 360 : 0 }}
              transition={{ repeat: Infinity, duration: 1.6, ease: "linear" }}
              className="w-10 h-10 rounded-full border-2 border-[#5c5344] bg-[#2d2820] flex items-center justify-center relative shadow-inner"
            >
            ...
            {/* Center Tape Window */}
            <div className="w-14 h-6 bg-[#0e0d0c] rounded border border-[#3f392f] flex items-center justify-center relative overflow-hidden">
              <div className="w-10 h-2 bg-[#42220f] rounded-sm opacity-90" />
            </div>
